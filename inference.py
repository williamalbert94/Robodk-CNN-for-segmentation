	...
	with tf.Session() as sess:
		...
		for ind, row in geodataframe.T.iteritems():
			...
			if args.save_plot is True:
				...
				plt.imshow(result)#[np.min(COORD[0]):np.max(COORD[0]),np.min(COORD[1]):np.min(COORD[1])+200,:])
				plt.axis('off')
				plt.savefig('{}/{}_{}_inf.png'.format(Path_image,name[0],name[1]), bbox_inches='tight',dpi=100)
				ret, thresh = cv2.threshold(mask, 0, 1, cv2.THRESH_BINARY)
				contours, hierarchy = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
				cv2.drawContours(result, contours, -1, (255,0,0),1 )

				#plt.figure(figsize=(30,30))
				#plt.title('Img/mask',fontsize=20)
				#plt.imshow(result)#[np.min(COORD[0]):np.max(COORD[0]),np.min(COORD[1]):np.min(COORD[1])+200,:])
				#plt.axis('off')
				#plt.savefig('{}/{}_{}_inf.png'.format(Path_border,name[0],name[1]), bbox_inches='tight',dpi=100)

				plt.figure(figsize=(30,30))
				plt.title('Reference',fontsize=20)
				plt.imshow(mask*255)#[np.min(COORD[0]):np.max(COORD[0]),np.min(COORD[1]):np.min(COORD[1])+200]*255)
				plt.axis('off')
				plt.savefig('{}/{}_{}_inf.png'.format(Path_mask,name[0],name[1]), bbox_inches='tight',dpi=100)

				plt.figure(figsize=(30,30))
				plt.title('Inference',fontsize=20)
				plt.imshow(reconstructed_heatmp)#[np.min(COORD[0]):np.max(COORD[0]),np.min(COORD[1]):np.min(COORD[1])+200])
				plt.axis('off')
				plt.savefig('{}/{}_{}_inf.png'.format(Path_Inference,name[0],name[1]), bbox_inches='tight',dpi=100)
...
def main(args):
	...
	#geodataframe = pd.read_csv('/scratch/parceirosbr/manntisict/radar/ia-arqgis/segmentation/experiments_report/RLE_FILE_QLK__TEST.csv')
	#geodataframe = pd.read_csv('/scratch/parceirosbr/manntisict/radar/ia-arqgis/segmentation/experiments_report/QLK_USE_GTMCORRECTED_2018-2019.csv')QLK_USE_GTMCORRECTED_2018-2019_TEST_DF
	geodataframe = pd.read_csv('/scratch/parceirosbr/manntisict/radar/experiencias/Segmentation/CSV/RLE/18-08-2021/QLK_USE_GTMCORRECTED_2018-2019_TEST_DF.csv')	
	geodataframe = geodataframe.dropna()
	index_dataset = list(range(len(geodataframe)))
	test_size = int((len(index_dataset)*70)/100)
